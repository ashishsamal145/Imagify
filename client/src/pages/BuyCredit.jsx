...
const BuyCredit = () => {
  ...
  const { currency, user, backendUrl, loadCreditsData, token, setShowLogin } = useContext(AppContext)
...

        {
          plans.map((item, index) => (
            <div key={index} className=' bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500'>
              ...
              <p className='mt-6'> <span className='text-3xl font-medium'>{currency}{item.price}</span>  / {item.credits} credits</p>
